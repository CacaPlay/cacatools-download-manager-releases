#!/usr/bin/env python3
"""Build the deterministic Phase 18 visual handoff gallery."""
...
MAIN = ROOT / "docs" / "screenshots" / "phase18"
DIALOGS = ROOT / "docs" / "screenshots" / "phase18-dialogs"
...
def main() -> int:
    ...
    if missing:
        raise SystemExit("Faltan capturas Phase 18:\n" + "\n".join(missing))
    canvas = Image.new("RGB", (CANVAS_WIDTH, CANVAS_HEIGHT), (6, 10, 15))
    draw = ImageDraw.Draw(canvas)
    draw.text((MARGIN, 34), "CacaTools Download Manager · Fase 18", font=font(42, True), fill=(242, 246, 250))
    draw.text((MARGIN, 88), "Zen predeterminado, Command Center rediseñado, búsqueda fluida y diálogos centrados.", font=font(22), fill=(148, 160, 176))
    ...
    print(f"OK: galería Phase 18 creada en {OUTPUT.relative_to(ROOT)} ({CANVAS_WIDTH}x{CANVAS_HEIGHT}).")
    return 0
