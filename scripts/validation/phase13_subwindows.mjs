...
requireToken(rust, 'resizable(true)', 'redimensionado nativo');
requireToken(rust, 'remembered_size', 'tamaÃ±o inicial adaptativo y recordado');
...
requireToken(frontend, 'APPEARANCE_METRICS_STORAGE_KEY', 'escala compartida con el gestor');
...
requireToken(frontend, '--sp-root-font-size', 'tipografÃ­a alineada con el gestor');
...
if (frontend.includes('<small>${escapeHtml(state.source')) {
  failures.push('el enlace de origen se duplica debajo del tÃ­tulo de la subventana');
}
