...
const previewSvg = fs.readFileSync(path.join(root, 'app-ui/media-preview.svg'));
const previewData = `data:image/svg+xml;base64,${previewSvg.toString('base64')}`;
const cache = new Map();
...
function moduleUrl(filename) {
  ...
  if (file.endsWith(`${path.sep}app-ui${path.sep}main.js`)) {
    source = source
      ...
      .replaceAll('./app-ui/media-preview.svg', previewData)
...
if (previewMode) {
  ...
  if (previewDialog === 'playlist' && previewPlaylistStage === 'queue') {
    ...
    appState.playlistRuntime = {
      ...
      next: { job_id: 404, title: 'Jace June — Come Home', creator: 'Jace June', thumbnail: ${JSON.stringify(previewData)}, duration_label: '2:49', status: 'queued', progress: 0, detail: 'En espera', error: '' }
    };
  }
}
`;
...
css = css.replace("@import url('./download-manager/styles.css');", fs.readFileSync(path.join(root, 'app-ui/download-manager/styles.css'), 'utf8'));
