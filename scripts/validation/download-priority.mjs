...
// The HTTP worker is intentionally changed by the separately-authorized
// integrity consolidation. Priority's dispatcher/DTO/UI contracts above are
// still checked; the unrelated transport files remain protected here.
const protectedPaths = [
  'extension',
  'src-tauri/src/downloads/bandwidth.rs',
  'src-tauri/src/downloads/segmented.rs',
  'src-tauri/src/downloads/aria2.rs',
  'src-tauri/src/media/download/worker.rs'
];
const protectedDiff = execFileSync('git', ['diff', '--name-only', 'ec3366e01734680991b0614928cc556df72554df', '--', ...protectedPaths], { cwd: root, encoding: 'utf8' }).trim();
check(`Feature 03 o extensión modificada: ${protectedDiff}`, protectedDiff === '');
