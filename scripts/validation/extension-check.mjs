...
if (extensionCompatibility.minimumAppVersion !== '0.24.1' || extensionCompatibility.maximumTestedAppVersion !== '0.45.x') errors.push('app-compat.json no conserva el contrato pendiente aprobado');
...
for (const file of ['service-worker.js', 'sidepanel.js', 'thumbnail-service.js', 'content/detector.js']) {
...
for (const token of ['chrome.downloads.onCreated', 'chrome.downloads.onChanged', 'chrome.downloads.onErased', 'pause', 'resume', 'cancel', 'CAPTURE_ACCEPTED', 'CAPTURE_FALLBACK', 'browser_download_capture']) {
