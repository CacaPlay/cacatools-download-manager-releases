...
const segmented = read('src-tauri/src/downloads/segmented.rs');
const aria = read('src-tauri/src/downloads/aria2.rs');
...
check('Adapter aria2 ausente', bandwidth.includes('apply_to_aria2') && bandwidth.includes('--max-download-limit='));
check('Adapter curl ausente', bandwidth.includes('apply_to_curl'));
...
check('HTTP segmentado no comparte un limiter', segmented.includes('bandwidth_limiter: Option<Arc<BandwidthLimiter>>') && segmented.includes('let bandwidth_limiter = bandwidth_limiter.clone()'));
check('aria2 HTTP no aplica policy', aria.includes('bandwidth_policy.apply_to_aria2'));
check('aria2 torrent no aplica policy', torrent.includes('bandwidth_policy.apply_to_aria2'));
...
// Keep the protected-scope comparison anchored to the reviewed candidate
// commit.  The old hash was not present in this clean repository, so using
// the existing candidate preserves the intended committed-change guard.
const protectedDiff = execFileSync('git', [
  'diff', 'd681dc874110d1729319c0a8d4090c33b209c2f7', '--',
...
console.log('OK: bandwidth policy, engine adapters, closed IPC, Settings UI and protected scopes validated.');
