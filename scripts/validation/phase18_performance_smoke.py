#!/usr/bin/env python3
"""Performance and compact-layout gate for the Phase 18 download manager."""
...
REPORT = ROOT / "docs" / "tests" / "phase18-performance-smoke.json"
MODULES = (
...
def main() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        unified.focus()
        page.evaluate("window.__phase18UnifiedNode = document.querySelector('[data-dm-unified-input]')")
        ...
        search_state = page.evaluate("""() => {
          ...
          return { same:input===window.__phase18UnifiedNode, focused:document.activeElement===input, value:input.value, inputBorder:getComputedStyle(input).borderTopWidth, wrapperBorder:getComputedStyle(wrap).borderTopWidth };
        }""")
        ...
        layout_select.select_option("command-center")
        page.wait_for_function("document.querySelector('.dm-host')?.dataset.dmLayout === 'command-center'")
        layout_ms = float(page.evaluate("performance.now()")) - layout_started
        zen_rows = page.locator(".dm-download-item").count()
        check("Cambio real a Command Center conserva el filtro", 160 <= zen_rows <= 170, zen_rows)
        ...
        sidebar_rect = page.locator(".dm-command-sidebar").evaluate(
            "node => { const r=node.getBoundingClientRect(); return {left:r.left,right:r.right,width:r.width}; }"
        )
        check("Sidebar móvil del layout activo se abre dentro de la pantalla", sidebar_rect["left"] >= -1 and sidebar_rect["right"] <= 801, sidebar_rect)
    ...
    report = {
        "gate": "phase18-performance-smoke",
        "passed": all(item["pass"] for item in checks),
        "sourceFiles": source_files,
        "sourceHash": source_hash,
        "queueSize": 1000,
        "thresholdsMs": {"initialRender": 2800, "filter": 1200, "suggestions": 1900, "layoutSwitch": 1800},
        "measurementsMs": {
    ...
    if failed:
        ...
        return 1
    print(
        "OK: Phase 18 con 1,000 trabajos "
