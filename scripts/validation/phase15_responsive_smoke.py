...
icon='data:image/svg+xml;base64,'+base64.b64encode((root/'app-ui/favicon.svg').read_bytes()).decode(); media='data:image/svg+xml;base64,'+base64.b64encode((root/'app-ui/media-preview.svg').read_bytes()).decode()
def html(query):
  js=js0.replace("const qs = new URLSearchParams(window.location.search);", f"const qs = new URLSearchParams({query!r});",1).replace('./app-ui/favicon.svg',icon).replace('./app-ui/media-preview.svg',media)
