...
const previewSvg = fs.readFileSync(path.resolve('app-ui/media-preview.svg'));
const previewData = `data:image/svg+xml;base64,${previewSvg.toString('base64')}`;

const [,, layout = 'command-center', theme = 'dark', output = ''] = process.argv;
const preferences = {
  layout, theme, accent: '#2f9bff', success: '#45cf89', warning: '#e6ad48', danger: '#ef6975',
  sidebarCollapsed: true, inspectorCollapsed: true, lowerPanelCollapsed: true,
  compactRows: false, filter: 'all', category: 'all', query: '', selectedJobId: 1,
  inspectorTab: 'summary', commandPanel: 'overview', section: 'downloads', uiScale: 118, appearanceRevision: 3
};
...
const markup = renderDownloadManager({
  ...
  downloadDirectory: 'C:\\Users\\jerem\\Downloads\\CDM', previewMode: true
});
