(function installCacaToolsDetector() {
  ...
  const providerName = () => {
    ...
    if (/(^|\.)spotify\.com$/i.test(host) || /(^|\.)scdn\.co$/i.test(host)) return 'spotify';
  ...
  function collect() {
    seen.clear();
    if (/(^|\.)spotify\.com$/i.test(location.hostname) || /(^|\.)scdn\.co$/i.test(location.hostname)) {
      const canonicalUrl = safeUrl(document.querySelector('link[rel="canonical"]')?.href) || location.href;
      const path = new URL(canonicalUrl).pathname;
      return {
        status: 'media_found',
        detections: [{
          id: 'spotify-page',
          type: /\/(?:album|playlist)\//i.test(path) ? 'playlist' : 'audio',
          ...
          thumbnail: pageImage(),
          confidence: 100,
          sourceKind: /\/(?:album|playlist)\//i.test(path) ? 'playlist' : 'provider_page',
          provider: 'spotify'
        }]
      };
    }
