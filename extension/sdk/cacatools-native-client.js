...
export const SPOTIFY_DIRECT_CAPTURE_MESSAGE = 'Las capturas directas de Spotify no se tratan como archivos HTTP.';
...
export function isSpotifyUrl(value) {
  try {
    ...
    return host === 'spotify.com' || host.endsWith('.spotify.com') || host === 'scdn.co' || host.endsWith('.scdn.co');
  } catch {
    return false;
  }
}
...
export const cacaToolsNative = Object.freeze({
  ...
  captureDownload: (capture) => isSpotifyUrl(capture?.finalUrl || capture?.url)
    ? Promise.resolve({ ok: false, status: 'spotify_direct_capture_ignored', error: SPOTIFY_DIRECT_CAPTURE_MESSAGE })
    : sendCacaToolsNativeMessage('browser_download_capture', capture)
});
