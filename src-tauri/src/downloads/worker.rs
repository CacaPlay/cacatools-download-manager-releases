...
use super::{
    ...
    current_downloads_dir, downloaded_file_looks_like_html, fail_job, finalize_http_job,
    ...
    persist_http_representation, release_http_lease, remove_http_segment_artifacts,
...
use crate::progress::adapter::{
    shadow_begin_http, shadow_http_observe, shadow_http_response_headers, shadow_http_start_stream,
    shadow_mark_http_cancelled, shadow_mark_http_cancelling, shadow_mark_http_completed,
    shadow_mark_http_failed, shadow_mark_http_finalizing, shadow_mark_http_paused,
    shadow_mark_http_post_processing, shadow_mark_http_preparing,
};
use crate::{
    background_command, configure_connection, ensure_public_network_resolution, kill_process_tree,
    parse_public_http_url, register_external_process, reject_spotify_source,
    url_has_public_network_target, DownloadQueueReceipt, ExternalProcessKind,
    ExternalProcessRegistry, LocalState, ProgressPersistenceGate, TransferRateSampler,
...
const CURL_IDLE_TIMEOUT_SECS: u64 = 60;
const CURL_STARTUP_RETRIES: u8 = 3;
...
fn download_temp_size(db_path: &Path, id: i64) -> u64 {
    Connection::open(db_path)
        ...
        .and_then(|path| PathBuf::from(path).metadata().ok())
        .map(|metadata| metadata.len())
        .unwrap_or(0)
}

pub(super) fn schedule_aria2_resume(db_path: &Path, id: i64) -> Result<bool, String> {
    ...
    let attempts: i64 = connection
        .query_row(
            "SELECT COALESCE(http_engine_attempts,0) FROM download_jobs WHERE job_id=?1",
            params![id],
            |row| row.get(0),
        )
        .unwrap_or(0);
    if attempts >= aria2::ARIA2_MAX_RESUME_RETRIES {
        return Ok(false);
    }
    let next_attempt = attempts + 1;
    let detail = format!(
        "Reintentando HTTP con el parcial aria2c ({next_attempt}/{})…",
        aria2::ARIA2_MAX_RESUME_RETRIES
    );
    ...
    let changed = transaction
        .execute(
            "UPDATE jobs SET status='queued',detail=?1,updated_at=CURRENT_TIMESTAMP WHERE id=?2 AND status IN ('queued','running')",
            params![detail, id],
        )
        .map_err(|error| error.to_string())?;
    if changed == 0 {
    ...
    transaction
        .execute(
            "UPDATE download_jobs SET http_engine_attempts=?1,error=NULL,updated_at=CURRENT_TIMESTAMP WHERE job_id=?2",
            params![next_attempt, id],
        )
        .map_err(|error| error.to_string())?;
    transaction.commit().map_err(|error| error.to_string())?;
    // This is a worker thread, not Tauri's UI or async dispatcher thread.
    thread::sleep(Duration::from_secs(1_u64 << (next_attempt.min(3) as u32)));
    Ok(true)
}
...
pub(crate) fn run_download_worker_with_completion(
    ...
    run_download_worker_with_engine(db_path, active, id, completion, None)
}

pub(crate) fn run_download_worker_with_engine(
    db_path: PathBuf,
    active: Arc<Mutex<HashSet<i64>>>,
    id: i64,
    completion: Option<WorkerCompletion>,
    ...
    run_download_worker_with_engine_and_registry(db_path, active, id, completion, aria2_path, None)
}

pub(crate) fn run_download_worker_with_engine_and_registry(
    db_path: PathBuf,
    active: Arc<Mutex<HashSet<i64>>>,
    id: i64,
    completion: Option<WorkerCompletion>,
    ...
    thread::spawn(move || {
        ...
        if let Err(primary_error) = run_download_worker_inner_with_engine(
            &db_path,
            id,
            aria2_path.as_deref(),
            external_processes.as_ref(),
        ) {
...
pub(crate) fn run_curl_download_worker_inner(
    db_path: &Path,
    id: i64,
    external_processes: Option<&ExternalProcessRegistry>,
) -> Result<(), String> {
    ...
    configure_connection(&connection).map_err(|error| error.to_string())?;
    let bandwidth_policy = super::read_job_bandwidth_policy(&connection, id);
    let (url, destination, temp_path, total_bytes): (String, String, String, Option<i64>) =
        connection
            .query_row(
                "SELECT url,destination,temp_path,total_bytes FROM download_jobs WHERE job_id=?1",
                params![id],
                |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?)),
            )
            .map_err(|error| error.to_string())?;
    let parsed_url = parse_public_http_url(&url, "El enlace de descarga no es válido")?;
    let request_url = canonical_google_docs_export_url(&parsed_url).unwrap_or(parsed_url.clone());
    let url = request_url.to_string();
    let parsed_url = request_url;
    ensure_public_network_resolution(&parsed_url)?;
    ...
    connection
        .execute(
            "UPDATE jobs SET status='running',detail='Motor HTTP alternativo · conectando…',updated_at=CURRENT_TIMESTAMP WHERE id=?1",
            params![id],
        )
        .map_err(|error| error.to_string())?;
    ...
    let curl_idle_timeout = CURL_IDLE_TIMEOUT_SECS.to_string();
    command
        .args([
            "--proto",
            "=http,https",
    ...
    bandwidth_policy.apply_to_curl(&mut command);
    command
        .arg("--referer")
        .arg(parsed_url.origin().ascii_serialization())
        .arg("--output")
        .arg(&temp_path)
        .arg(&url)
        .stdout(Stdio::null())
    ...
    let mut child = command
        .spawn()
        .map_err(|error| format!("No se pudo iniciar el respaldo curl local: {error}"))?;
    let pid = child.id();
    let _process_guard = if let Some(registry) = external_processes {
        match register_external_process(registry, id, pid, ExternalProcessKind::HttpFallback) {
            Ok(guard) => Some(guard),
            Err(error) => {
    ...
    let initial_size = temp_path.metadata().map(|value| value.len()).unwrap_or(0);
    ...
    shadow_http_start_stream(
        ...
        initial_size,
        total_bytes,
        http_v1_observation(initial_size, total_bytes, None, None, "running", None),
    );
    let mut speed_sampler = TransferRateSampler::new(initial_size);
    let mut progress_gate = ProgressPersistenceGate::new();
    let mut last_progress_size = initial_size;
    ...
    loop {
        ...
        if matches!(status.as_str(), "paused" | "cancelled") {
            ...
            if status == "cancelled" {
                ...
                connection
                    .execute(
                        "UPDATE jobs SET progress=0,detail=?1,updated_at=CURRENT_TIMESTAMP WHERE id=?2",
                        params![detail, id],
                    )
                    .map_err(|error| error.to_string())?;
            } else {
                connection
                    .execute(
                        "UPDATE jobs SET detail='En pausa · parcial conservado por el motor alternativo',updated_at=CURRENT_TIMESTAMP WHERE id=?1",
                        params![id],
                    )
                    .map_err(|error| error.to_string())?;
            }
        ...
        if let Some(exit) = child.try_wait().map_err(|error| error.to_string())? {
            ...
            if !exit.success() {
                let detail = stderr_output.trim().chars().take(700).collect::<String>();
                return Err(if detail.is_empty() {
                    "curl terminó sin completar el archivo".into()
                } else {
                    detail
                });
            }
        ...
        thread::sleep(Duration::from_millis(DOWNLOAD_PROGRESS_UPDATE_INTERVAL_MS));
        let current_size = temp_path.metadata().map(|value| value.len()).unwrap_or(0);
        if current_size > last_progress_size {
            last_progress_size = current_size;
            last_progress_at = Instant::now();
        } else if last_progress_at.elapsed() >= Duration::from_secs(CURL_IDLE_TIMEOUT_SECS) {
            let _ = connection.execute(
                "UPDATE jobs SET detail='Conexión estancada · reintentando desde el parcial…',updated_at=CURRENT_TIMESTAMP WHERE id=?1 AND status='running'",
                params![id],
            );
            kill_process_tree(pid);
            let _ = child.wait();
            let _ = stderr_reader.join();
            return Err(format!(
                "curl quedó sin recibir datos durante {CURL_IDLE_TIMEOUT_SECS} segundos desde {}",
                human_bytes(current_size)
            ));
        }
        let speed = speed_sampler.sample(current_size);
        let total = total_bytes;
        let eta = total.and_then(|value| {
            (speed > 1.0 && value > current_size)
                .then_some(((value - current_size) as f64 / speed).ceil() as u64)
        });
        shadow_http_observe(
            id,
            current_size,
        ...
        let progress = total
            .map(|value| current_size as f64 * 100.0 / value as f64)
            .unwrap_or(0.0)
            .clamp(0.0, 99.4);
        if !progress_gate.should_persist(current_size, progress, Duration::ZERO) {
            continue;
        }
        let eta = eta.map(|value| value as i64);
        let detail = match total {
            Some(value) => format!(
                "{} de {} · {}/s · motor alternativo",
                human_bytes(current_size),
                human_bytes(value),
                human_bytes(speed as u64)
            ),
            None => format!(
                "{} descargados · {}/s · motor alternativo",
                human_bytes(current_size),
                human_bytes(speed as u64)
            ),
        };
        ...
        transaction
            .execute(
                "UPDATE download_jobs SET downloaded_bytes=?1,speed_bps=?2,eta_seconds=?3,updated_at=CURRENT_TIMESTAMP WHERE job_id=?4",
                params![current_size as i64, speed, eta, id],
            )
            .map_err(|error| error.to_string())?;
        transaction.commit().map_err(|error| error.to_string())?;
        progress_gate.mark_persisted(current_size, progress);
    }

    shadow_mark_http_post_processing(id);
    let final_size = temp_path
        .metadata()
        .map_err(|error| format!("curl no produjo un archivo final: {error}"))?
        .len();
    if final_size == 0 {
        return Err("curl produjo un archivo vacío".into());
    }
    if downloaded_file_looks_like_html(&temp_path) {
        return Err("El enlace devolvió HTML en lugar del archivo solicitado".into());
    }
    if let Some(total) = total_bytes.filter(|value| *value > 0) {
        if final_size != total {
            return Err(format!(
                "El respaldo local recibió {} de {}",
                human_bytes(final_size),
                human_bytes(total)
            ));
        }
    }
    ...
    fs::rename(&temp_path, &destination)
        .map_err(|error| format!("No se pudo finalizar el archivo descargado por curl: {error}"))?;
...
fn run_curl_download_worker_with_retries(
    db_path: &Path,
    id: i64,
    external_processes: Option<&ExternalProcessRegistry>,
) -> Result<(), String> {
    let mut last_error = String::new();
    for attempt in 0..CURL_STARTUP_RETRIES {
        if attempt > 0 {
            let connection = Connection::open(db_path).map_err(|error| error.to_string())?;
            let _ = connection.execute(
                ...
                params![
                    format!(
                        "Motor HTTP alternativo · reintentando conexión ({}/{})…",
                        attempt + 1,
                        CURL_STARTUP_RETRIES
                    ),
                    id
                ],
            );
        ...
        match run_curl_download_worker_inner(db_path, id, external_processes) {
            Ok(()) => return Ok(()),
            Err(error) => {
                last_error = error;
                if !should_retry_curl_attempt(&last_error) {
                    break;
                }
            }
        }
...
fn should_retry_curl_attempt(error: &str) -> bool {
...
pub(crate) fn run_download_worker_inner(db_path: &Path, id: i64) -> Result<(), String> {
    run_download_worker_inner_with_engine(db_path, id, None, None)
}
...
fn run_download_worker_inner_with_engine(
    db_path: &Path,
    id: i64,
    _aria2_path: Option<&Path>,
    _external_processes: Option<&ExternalProcessRegistry>,
) -> Result<(), String> {
...
fn run_rust_http_worker(db_path: &Path, id: i64, lease: &super::HttpLease) -> Result<(), String> {
    ...
    let client = Client::builder()
        ...
        .timeout(Duration::from_secs(CURL_IDLE_TIMEOUT_SECS))
...
mod destination_reservation_tests {
    ...
    #[test]
    fn rapid_http_reservations_get_distinct_partial_paths() {
