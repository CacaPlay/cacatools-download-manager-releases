...
impl BandwidthPolicy {
    ...
    pub(crate) fn apply_to_curl(&self, command: &mut Command) {
        if let Some(value) = self.bytes_per_second() {
            command.arg("--limit-rate").arg(value.to_string());
        }
    }
...
mod tests {
    ...
    #[test]
    fn engine_adapters_use_exact_decimal_bytes_and_unlimited_adds_nothing() {
        ...
        limited.apply_to_curl(&mut curl);
        assert_eq!(args(&curl), ["--limit-rate", "5000000"]);
        ...
        BandwidthPolicy::Unlimited.apply_to_curl(&mut unlimited);
