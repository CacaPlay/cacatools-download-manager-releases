...
pub(crate) async fn resolve_spotify_source(
    url: String,
    app: AppHandle,
    db_path: PathBuf,
) -> Result<MediaAnalysisSnapshot, String> {
    tauri::async_runtime::spawn_blocking(move || {
        super::spotify::resolve_spotify_analysis_sync(&url, &app, &db_path)
    })
    .await
    .map_err(|error| format!("No se pudo resolver Spotify: {error}"))?
}
