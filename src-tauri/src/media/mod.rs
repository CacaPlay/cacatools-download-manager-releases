...
pub(crate) use analysis::{
    ...
    analyze_media_url_with_session_options, resolve_spotify_source,
};
pub(crate) use download::*;
pub(crate) use models::{
    MediaAnalysisSnapshot, MediaFormatSnapshot, MediaItemSnapshot, SpotifyCandidateSnapshot,
};
...
pub(crate) use queue::{
    ...
    queue_media_download_secure, queue_spotify_download, MediaQueueReceipt,
};
pub(crate) use recovery::recover_media_source;
...
pub(crate) use spotify::resolve_spotify_analysis_sync;
...
pub(crate) fn next_playlist_job(connection: &Connection, batch_id: i64) -> Option<i64> {
    connection.query_row(
        "SELECT job_id FROM playlist_items WHERE batch_id=?1 AND status='queued' AND job_id IS NOT NULL ORDER BY position LIMIT 1",
        params![batch_id],
        |row| row.get(0),
    ).optional().ok().flatten()
}
...
pub(crate) fn start_playlist_jobs(
    ...
    let Ok(rows) = statement.query_map(params![batch_id], |row| row.get::<_, i64>(0)) else {
        return;
    };
...
pub(crate) fn advance_playlist_batch(
    ...
    let queued: i64 = connection
        ...
        .unwrap_or(0);
    if queued > 0 && running < SPOTDL_PLAYLIST_CONCURRENCY as i64 {
        drop(connection);
        ...
        let retryable_jobs = {
            ...
            rows.flatten()
                .filter(|(_, error, provider)| {
                    let diagnosis = failure_diagnosis(error, provider);
    ...
    match &state.media_runtime {
        Some(runtime) => {
            ...
            MediaRuntimeSnapshot {
                ...
                ffprobe_version,
                spotdl: false,
                detail: "Motor multimedia local disponible; Spotify está desactivado temporalmente"
                    .into(),
            }
        }
