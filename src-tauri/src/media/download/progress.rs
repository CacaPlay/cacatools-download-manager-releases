...
pub(crate) fn update_spotdl_progress(connection: &Connection, id: i64, bytes: &[u8]) {
    if connection
        .query_row("SELECT status FROM jobs WHERE id=?1", params![id], |row| {
            row.get::<_, String>(0)
        })
    ...
    let stage = if lower.contains("searching for song") {
        Some((8.0, "Buscando coincidencia con spotDL"))
    } else if lower.contains("getting audio meta") {
        Some((16.0, "Resolviendo metadatos de audio"))
    } else if lower.contains("downloading") {
        Some((35.0, "Descargando audio"))
    } else if lower.contains("embedding metadata") {
        Some((92.0, "Aplicando metadatos"))
    } else if lower.contains("done") || lower.contains("complete") {
        Some((95.0, "Audio recibido; validando archivo"))
    } else {
        None
    };
    let Some(percent_end) = line.find('%') else {
        if let Some((progress, detail)) = stage {
            let _ = connection.execute(
                "UPDATE jobs SET progress=?1,detail=?2,updated_at=CURRENT_TIMESTAMP WHERE id=?3 AND status='running'",
    ...
    let start = prefix
        ...
        .unwrap_or(0);
    let value = prefix[start..].replace(',', ".").parse::<f64>().ok();
    let Some(percent) = value.filter(|value| value.is_finite()) else {
        return;
    };
    let progress = (3.0 + percent.clamp(0.0, 95.0) * 0.94).min(96.0);
    let detail = format!("Descargando con spotDL · {:.0}%", percent.clamp(0.0, 100.0));
    let _ = connection.execute(
        "UPDATE jobs SET progress=?1,detail=?2,updated_at=CURRENT_TIMESTAMP WHERE id=?3 AND status='running'",
        params![progress, detail, id],
    );
