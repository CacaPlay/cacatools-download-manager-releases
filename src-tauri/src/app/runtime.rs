...
use crate::{background_command, SPOTDL_SHA256};
...
pub(crate) struct MediaRuntimePaths {
    ...
    pub(crate) spotdl: Option<PathBuf>,
}
...
pub(crate) fn discover_media_runtime(app: &AppHandle) -> Option<MediaRuntimePaths> {
    ...
    // La arquitectura de Spotify se conserva para una reactivación futura,
    // pero 0.24.2 no descubre, valida ni ejecuta spotDL durante el inicio.
    Some(MediaRuntimePaths {
        yt_dlp,
        ffmpeg_dir,
        spotdl: None,
    })
...
pub(crate) fn verify_spotdl_binary(path: &Path) -> Result<(), String> {
    if !path.is_file() {
        return Err("spotify_engine_unavailable".into());
    }
    let bytes = fs::read(path).map_err(|_| "spotify_engine_unavailable".to_string())?;
    let digest = Sha256::digest(bytes);
    let actual = format!("{digest:x}");
    if !actual.eq_ignore_ascii_case(SPOTDL_SHA256) {
        return Err("spotify_engine_hash_mismatch".into());
    }
    Ok(())
}
