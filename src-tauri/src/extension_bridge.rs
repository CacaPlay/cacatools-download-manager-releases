    ...
    match action.as_str() {
        ...
        "enqueue" | "analyze" | "browser_download_capture" => {
            ...
            if action == "browser_download_capture" && is_spotify_source(source) {
                return json!({
                    "ok": false,
                    "status": "spotify_direct_capture_ignored",
                    "error": SPOTIFY_DIRECT_CAPTURE_MESSAGE
                });
            }
...
mod tests {
    ...
    #[test]
    fn spotify_source_is_allowed_for_multimedia_routing() {
        assert!(is_spotify_source("https://open.spotify.com/track/abc123"));
        assert!(is_spotify_source("spotify:playlist:abc123"));
        assert!(!is_spotify_source("https://www.youtube.com/watch?v=abc123"));
    }
