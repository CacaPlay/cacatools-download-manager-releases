...
export function newsPanel(context = {}) {
  ...
  const markup = filtered.map((message) => {
    ...
    const media = image ? `<button class="dm-news-image-button" type="button" data-dm-news-action="open-news-image" data-news-image="${escapeHtml(image)}"><img class="dm-news-thumbnail" src="${escapeHtml(image)}" alt="" loading="lazy" decoding="async"></button>` : asset ? `<img class="dm-news-label-icon" src="${asset}" alt="" loading="eager" decoding="async">` : dmIcon(iconFor(message), 22);
    ...
    return `<article class="dm-news-item ${message.actionRequired ? 'dm-news-action' : ''} ${message.read ? 'is-read' : ''}" data-news-id="${escapeHtml(message.id)}"><span class="dm-news-media">${media}</span><div class="dm-news-copy"><div class="dm-news-meta">${category ? `<span class="dm-news-category">${escapeHtml(category)}</span>` : ''}${date ? `<span class="dm-news-date">${category ? '· ' : ''}${escapeHtml(date)}</span>` : ''}</div><strong>${escapeHtml(title)}</strong><small>${escapeHtml(summary)}</small>${actionsFor(message) ? `<div class="dm-news-actions">${actionsFor(message)}</div>` : ''}</div></article>`;
  }).join('');
