...
export function newsPanel(context = {}) {
  ...
  const markup = filtered.map((message) => {
    ...
    return `<article class="dm-news-item ${message.actionRequired ? 'dm-news-action' : ''} ${message.read ? 'is-read' : ''}" data-news-id="${escapeHtml(message.id)}"><span class="dm-news-media">${media}</span><div class="dm-news-copy"><small class="dm-news-meta">${message.type === 'update' ? t('updateCategory', 'ACTUALIZACIÓN') : message.type === 'extension' ? t('extensionCategory', 'EXTENSIÓN') : ''}${date ? ` · ${escapeHtml(date)}` : ''}</small><strong>${escapeHtml(title)}</strong><small>${escapeHtml(summary)}</small>${actionsFor(message) ? `<div class="dm-news-actions">${actionsFor(message)}</div>` : ''}</div></article>`;
  }).join('');
  ...
  const support = filter === 'history' ? '' : `<aside class="dm-news-support"><span>${dmIcon('heart', 22)}</span><div><strong>${t('support', 'Apoya el proyecto')}</strong><small>${t('supportDescription', 'Tu apoyo ayuda a mantener Clear Download Manager en desarrollo.')}</small></div><button type="button" data-dm-news-action="support">${t('supportAction', 'Apoyar')}</button></aside>`;
