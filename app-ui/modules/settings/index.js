...
function generalPage() {
  ...
  return `<div class="settings-page settings-page-general">${pageIntro(tr('general'), tr('windowStartup'))}<section class="settings-section">${sectionHeading('app', tr('windowBehavior'))}${selectControl('close-action-select', 'closeAction', tr('closeAction'), closeAction, [['tray', tr('closeToTray')], ['exit', tr('exitCompletely')]])}<label class="settings-control settings-switch-control"><span><b>${tr('startWithWindows')}</b></span><input id="startup-toggle" type="checkbox" ${checked(Boolean(appState.startupStatus?.enabled), true)} ${startupDisabled ? 'disabled' : ''}></label></section><section class="settings-section">${sectionHeading('globe', tr('language'))}<label class="settings-control"><span><b>${tr('language')}</b><small>${tr('system')} ${tr('system') === 'System' ? 'detects the Windows language.' : 'detecta el idioma de Windows.'}</small></span><select data-experience-field="locale" aria-label="${tr('language')}"><option value="system" ${selected(selectedLocale, 'system')}>${tr('system')}</option><option value="es" ${selected(selectedLocale, 'es')}>${tr('spanish')}</option><option value="en" ${selected(selectedLocale, 'en')}>${tr('english')}</option></select></label></section><section class="settings-section">${sectionHeading('bell', tr('automation'))}${experienceToggle('clipboard-suggest-toggle', 'clipboardAutoSuggest', tr('detectClipboard'), tr('clipboardDescription'), experience.clipboardAutoSuggest)}${experienceToggle('news-toggle', 'receiveNews', tr('receiveNews'), tr('receiveNewsDescription'), experience.receiveNews)}${experienceToggle('auto-updates-toggle', 'automaticUpdateChecks', tr('automaticUpdates'), tr('automaticUpdatesDescription'), experience.automaticUpdateChecks)}${experienceToggle('extension-recommendation-toggle', 'showExtensionRecommendation', tr('extensionRecommendation'), tr('extensionRecommendationDescription'), experience.showExtensionRecommendation)}</section><section class="settings-section settings-section-compact">${sectionHeading('shield', tr('status'))}${statusRow(tr('minimize'), tr('taskbar'))} ${statusRow(tr('closeAction'), closeAction === 'exit' ? tr('exit') : tr('tray'))}</section></div>`;
}
