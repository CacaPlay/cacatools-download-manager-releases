...
function generalPage() {
  ...
  return `<div class="settings-page settings-page-general">${pageIntro('General', 'Ventana e inicio')}<section class="settings-section">${sectionHeading('app', 'Comportamiento de ventana')}${selectControl('close-action-select', 'closeAction', 'Al pulsar cerrar', closeAction, [['tray', 'Cerrar a la bandeja'], ['exit', 'Salir completamente']])}<label class="settings-control settings-switch-control"><span><b>Iniciar con Windows</b></span><input id="startup-toggle" type="checkbox" ${checked(Boolean(appState.startupStatus?.enabled), true)} ${startupDisabled ? 'disabled' : ''}></label></section><section class="settings-section">${sectionHeading('globe', tr('language'))}<label class="settings-control"><span><b>${tr('language')}</b><small>${tr('system')} detecta el idioma de Windows.</small></span><select data-experience-field="locale" aria-label="${tr('language')}"><option value="system" ${selected(selectedLocale, 'system')}>${tr('system')}</option><option value="es" ${selected(selectedLocale, 'es')}>${tr('spanish')}</option><option value="en" ${selected(selectedLocale, 'en')}>${tr('english')}</option></select></label></section><section class="settings-section">${sectionHeading('bell', 'Automatismos')}${experienceToggle('clipboard-suggest-toggle', 'clipboardAutoSuggest', 'Detectar enlaces copiados', 'Sugerir un enlace al volver a enfocar CacaTools.', experience.clipboardAutoSuggest)}${experienceToggle('news-toggle', 'receiveNews', 'Recibir novedades', 'Muestra avisos y mensajes oficiales en Novedades.', experience.receiveNews)}${experienceToggle('auto-updates-toggle', 'automaticUpdateChecks', 'Buscar actualizaciones automáticamente', 'Comprueba actualizaciones en segundo plano cuando hay conexión.', experience.automaticUpdateChecks)}${experienceToggle('extension-recommendation-toggle', 'showExtensionRecommendation', 'Mostrar recomendación de la extensión', 'Permite que la extensión aparezca como sugerencia en Novedades.', experience.showExtensionRecommendation)}</section><section class="settings-section settings-section-compact">${sectionHeading('shield', 'Estado')}${statusRow('Minimizar', 'Barra de tareas')} ${statusRow('Cierre', closeAction === 'exit' ? 'Salida completa' : 'Bandeja')}</section></div>`;
}
...
function integrationsPage() {
  ...
  return `<div class="settings-page settings-page-integrations">${pageIntro('Integraciones', 'Conexiones disponibles')}<section class="settings-section">${sectionHeading('link', 'Extensión del navegador')}${statusRow('Puente', extension?.prepared ? 'Preparado' : 'No detectado', extension?.prepared ? 'ok' : 'warn')} ${statusRow('Protocolo', extension?.protocolVersion ? `v${extension.protocolVersion}` : '—')}</section><section class="settings-section settings-section-compact">${sectionHeading('audio', 'Spotify')}<div class="settings-callout">Spotify está desactivado temporalmente.</div><button type="button" class="spotify-connect" disabled aria-disabled="true">Spotify desactivado</button></section></div>`;
}

function updatesPage() {
  const visual = window.__cacatoolsVisualDiagnostics || visualDiagnosticsSnapshot();
  const tool = appState.toolUpdateStatus || {};
  // UNAVAILABLE is the honest state while the signed production catalog is
  // not configured; it is not evidence that the installed tool is obsolete.
  const labels = { CURRENT: 'Actualizado', AVAILABLE: 'Disponible', CHECKING: 'Comprobando…', DOWNLOADING: 'Descargando…', VERIFYING: 'Verificando…', INSTALLING: 'Instalando…', UPDATED: 'Actualizado', FAILED: 'No se pudo completar', OFFLINE: 'Sin conexión', UNAVAILABLE: 'Actualizaciones no configuradas' };
  ...
  return `<div class="settings-page settings-page-updates">${pageIntro('Actualizaciones y diagnóstico', 'Estado local')}<section class="settings-section">${sectionHeading('shield', 'Versiones')}${statusRow('Aplicación', escapeHtml(appState.runtimeStatus?.version || APP_VERSION), 'neutral')} ${statusRow('yt-dlp', appState.mediaRuntimeStatus?.yt_dlp ? 'Disponible' : 'No detectado', appState.mediaRuntimeStatus?.yt_dlp ? 'ok' : 'warn')} ${statusRow('FFmpeg', appState.mediaRuntimeStatus?.ffmpeg ? 'Disponible' : 'No detectado', appState.mediaRuntimeStatus?.ffmpeg ? 'ok' : 'warn')}</section><section class="settings-section settings-section-tools" data-tool-update-section>${sectionHeading('tools', 'Herramientas internas')}<p class="settings-section-note">Comprueba de forma segura el motor interno de multimedia.</p>${statusRow('yt-dlp', state, tone)}${tool.availableVersion ? statusRow('Versión disponible', escapeHtml(tool.availableVersion)) : ''}${statusRow('Última comprobación', escapeHtml(checkedLabel), 'info')}<div class="settings-inline-actions settings-tool-update-actions"><button type="button" class="tool-update-check" ${checking || updating ? 'disabled aria-disabled="true"' : ''}>${icon('history', 16)} Buscar ahora</button>${canUpdate ? `<button type="button" class="tool-update-apply">${icon('download', 16)} Actualizar</button>` : ''}</div></section><section class="settings-section settings-section-compact">${sectionHeading('tools', 'Diagnóstico visual')}${statusRow('Escala efectiva', `${visual.resolvedScale || 100}%`, 'neutral')} ${statusRow('Tipografía', `${escapeHtml(visual.font?.size || '16px')} · Segoe UI`, 'neutral')}<button type="button" class="copy-visual-diagnostics">${icon('clipboard', 16)} Copiar diagnóstico</button></section></div>`;
}
...
export function settingsMarkup() {
  ...
  return `<section class="settings-view settings-workspace"><div class="settings-workspace-frame"><header class="settings-workspace-toolbar"><button type="button" class="settings-back" title="Volver al gestor" aria-label="Volver al gestor">${icon('arrow', 18)}<span>Volver al gestor</span></button></header><div class="settings-workspace-layout">${nav(activeCategory)}<main class="settings-workspace-content">${page}</main></div></div></section>`;
}
